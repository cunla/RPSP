...
public class Client {
	...
	public void addVmToCG(String vmId, Long groupId,
		...
		for (ConsistencyGroupCopySettings currConsistencyGroupCopySettings : consistencyGroupCopySettingsList) {
			if (currConsistencyGroupCopySettings.getRoleInfo()
				...
				createVMParam.setTargetVirtualCenterUID(new VirtualCenterUID(
						replicaVcId));
		...
		VmReplicationSetParamSet vmReplicationSetParamSet = new VmReplicationSetParamSet(
				innerSet);
	...
	private boolean isCopyImageAccessEnabled(Long clusterId, Long groupId,
		...
		boolean res = false;
		List<ConsistencyGroupCopySettings> allCopiesSettings = consistencyGroupCopySettingsSet
				.getInnerSet();
	...
	public void failOver(Long clusterId, Long groupId, int copyId) {
		...
		GlobalCopyUID globalCopyUIDParam = new GlobalCopyUID(new ClusterUID(
				clusterId), copyId);
	...
	public void failoverGroupSetSubset(Long clusterId, Long groupSetId) {
		...
		List<ConsistencyGroupUID> groupsUIDs = consistencyGroupSetSettings
				.getGroupsUIDs();
	...
	public long createConsistencyGroup(String cgName, List<String> vmIds,
		...
		// replication sets
		List<VMReplicationSetParam> vmReplicationSets = replicateVmsParam
		...
		// create copies
		List<ConsistencyGroupCopyParam> copiesList = replicateVmsParam
				.getCopies();
		for (PackageConfig accountConfig : accountConfigList) {
		...
		// create links
		List<FullConsistencyGroupLinkPolicy> linkPoliciesList = replicateVmsParam
				.getLinks();
		ConsistencyGroupLinkPolicy remoteDefaultLinkPolicy = connector
				.getDefaultRemoteGroupLinkPolicy();
	...
	public ResourcePoolUID getRelevantResourcePool(long clusterId, String vcId,
		...
		List<ArrayManagementProviderSettings> arrayManagementProviderSettings = clusterSettings
				.getAmpsSettings();
		for (ArrayManagementProviderSettings currProviderSettings : arrayManagementProviderSettings) {
			if (currProviderSettings.getType().equals(
				...
				List<ArraySettings> managedArrays = currProviderSettings
						.getManagedArrays();
				for (ArraySettings currArraySettings : managedArrays) {
					...
					if (vcId.equals(serialNumber)) {
						List<ArrayResourcePoolSettings> resourcePools = currArraySettings
								.getResourcePools();
