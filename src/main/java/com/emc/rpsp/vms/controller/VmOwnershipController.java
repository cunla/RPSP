...
@Controller
public class VmOwnershipController {
	...
	@ResponseBody
	public ResponseEntity<List<VmOwnership>> findVms() {
		List<VmOwnership> vmownerships = vmOwnershipService.findAll();
		return new ResponseEntity<>(vmownerships, HttpStatus.OK);
	}

	@PreAuthorize("hasAuthority('ADMIN')")
	@RequestMapping(value = "/vmownership", params = { "accountId" }, method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public ResponseEntity<List<VmOwnership>> findAccountVms(
	...
	@ResponseBody
	public ResponseEntity<VmOwnership> findVm(@PathVariable("id") Long id) {
	...
	@RpspAudited
	@PreAuthorize("hasAuthority('ADMIN')")
	@RequestMapping(value = "/vmownership", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<VmOwnership> createVm(
			@RequestBody VmOwnership vmOwnership) {
		VmOwnership sameVmOwnership = vmOwnershipService.findByVmId(vmOwnership
				.getVmId());
		if (sameVmOwnership != null) {
			return new ResponseEntity<>(HttpStatus.CONFLICT);
		}

		Account account = accountService.findById(vmOwnership.getTenantId());
		if (account == null) {
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
		}
		VmOwnership createdVmOwnership = vmOwnershipService.create(vmOwnership);
		HttpHeaders httpHeaders = new HttpHeaders();
		httpHeaders.add(HttpHeaders.LOCATION, "vmownerships/"
				+ createdVmOwnership.getId());
		return new ResponseEntity<>(createdVmOwnership, httpHeaders,
				HttpStatus.CREATED);
	}

	@RpspAudited
	@PreAuthorize("hasAuthority('ADMIN')")
	@RequestMapping(value = "/vmownership/{id}", method = RequestMethod.DELETE)
	@ResponseBody
