...
public class GroupsProtectionController {
    ...
    @ResponseBody
    public
    @RpspAuditResult(AuditConsts.ADD_VM_TO_CG_RESULT)
    ResponseEntity<HttpStatus> addVmToCG(
        ...
        groupsProtectionService.addVmToCG(vmId, groupId, isCritical, sequenceNumber);
        ...
        if (backup) {
            ...
            backupApi.addVmToBackupSchedule(vmId, vmName, schedule);
        }
        return new ResponseEntity<>(HttpStatus.OK);
    }
    ...
    @ResponseBody
    public
    @RpspAuditResult(AuditConsts.REMOVE_VM_FROM_CG_RESULT)
    ResponseEntity<HttpStatus> removeVmsFromCG(
