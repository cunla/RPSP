...
public class AccountController {
	...
	@PreAuthorize("hasAuthority('ADMIN')")
	@RequestMapping(value = "/accounts", params = {"systemId"}, method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public ResponseEntity<List<Account>> findSystemAccounts(@RequestParam("systemId") Long systemId) {
	...
	@PreAuthorize("hasAuthority('ADMIN')")
	@RequestMapping(value = "/accounts", params = {"systemId"}, method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Account> createAccount(@RequestBody Account account, @RequestParam("systemId") Long systemId) {
		...
		httpHeaders.add(HttpHeaders.LOCATION, "accounts/system/" + systemId + "/account/" + createdAccount.getId());
		return new ResponseEntity<>(createdAccount, httpHeaders,
		        HttpStatus.CREATED);
	}
	...
	public ResponseEntity<Account> updateAccount(@RequestBody Account account, @PathVariable("id") Long id) {
		...
		Account sameAccount = accountService.findByName(account.getName());
		if (sameAccount != null
		        && sameAccount.getId() != existingAccount.getId()) {
			return new ResponseEntity<>(HttpStatus.CONFLICT);
		}
		account.setId(id);
